...
class VIDDemo(object):
    ...
    def run_on_image_folder_VID(self, image_folder, suffix='.JPEG', max_num_frames=None):
        ...
        for frame_id in tqdm(range(frame_seg_len)):
            ...
            if self.method == "base":
                ...
                # run inference on current image
                predictions = self.run_on_image(original_image, infos)[1]

                # load annotations
                if anno_folder is not None:
                    anno_file = pattern_anno % frame_id
                ...
                predictions, anno, image_analyzer = self.preprocess_data_for_analyzer(predictions, img_for_analyzer, anno_type='vid', anno=anno)
    ...
    def preprocess_data_for_analyzer(self, predictions, img, anno_type='vid', anno=None):
        ...
        try:
            if anno_type == 'vid':
                anno = convert_vid_annotations_to_Box(anno)
                anno.resize(image_shape)

        except:
            anno = None

        return predictions, anno, img
